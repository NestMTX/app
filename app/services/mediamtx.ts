...
export class MediaMTXService {
  ...
  async boot(logger: LoggerService, nat: NATService, ice: ICEService) {
    ...
    const updated: any = {
       ...
       */
      rtsp: true === env.get('MEDIA_MTX_RTSP_ENABLED', true) ? 'yes' : 'no',
      protocols: ['tcp', 'udp'],
       ...
       */
      rtmp: true === env.get('MEDIA_MTX_RTMP_ENABLED', false) ? 'yes' : 'no',
      rtmpAddress: `${env.get('HOST')}:${env.get('MEDIA_MTX_RTMP_PORT', 1935)}`,
      /**
       * Update the HLS configuration
       */
      hls: true === env.get('MEDIA_MTX_HLS_ENABLED', false) ? 'yes' : 'no',
       ...
       */
      webrtc: true === env.get('MEDIA_MTX_WEB_RTC_ENABLED', false) ? 'yes' : 'no',
       ...
       */
      srt: true === env.get('MEDIA_MTX_SRT_ENABLED', false) ? 'yes' : 'no',
